...
Three first-party middlewares ship from the `@katajs-framework/core` core entry: `cors`,
...
`maxSize` defaults to `DEFAULT_MAX_BODY_SIZE`, exported from `@katajs-framework/core`:

```ts
import { DEFAULT_MAX_BODY_SIZE } from '@katajs-framework/core'
...
```ts
import { REQUEST_ID_HEADER } from '@katajs-framework/core'
...
Everything on this page comes from the `@katajs-framework/core` core entry:
...
- [JWT auth](/reference/jwt) — the `@katajs-framework/core/jwt` `jwtAuth` middleware.
