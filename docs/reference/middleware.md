...
Three first-party middlewares ship from the `@katajs/core` core entry: `cors`,
...
`maxSize` defaults to `DEFAULT_MAX_BODY_SIZE`, exported from `@katajs/core`:

```ts
import { DEFAULT_MAX_BODY_SIZE } from '@katajs/core'
...
```ts
import { REQUEST_ID_HEADER } from '@katajs/core'
...
Everything on this page comes from the `@katajs/core` core entry:
...
- [JWT auth](/reference/jwt) — the `@katajs/core/jwt` `jwtAuth` middleware.
