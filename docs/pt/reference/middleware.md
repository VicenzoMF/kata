...
Três middlewares first-party vêm do core entry do `@katajs/core`: `cors`,
...
`maxSize` usa por padrão `DEFAULT_MAX_BODY_SIZE`, exportado de `@katajs/core`:
...
Tudo nesta página vem do core entry do `@katajs/core`:
...
- [JWT auth](/pt/reference/jwt) — o middleware `jwtAuth` de `@katajs/core/jwt`.
