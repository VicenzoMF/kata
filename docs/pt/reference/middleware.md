...
Três middlewares first-party vêm do core entry do `@katajs-framework/core`: `cors`,
...
`maxSize` usa por padrão `DEFAULT_MAX_BODY_SIZE`, exportado de `@katajs-framework/core`:
...
Tudo nesta página vem do core entry do `@katajs-framework/core`:
...
- [JWT auth](/pt/reference/jwt) — o middleware `jwtAuth` de `@katajs-framework/core/jwt`.
