...
eles para que o resto do app importe de `./context`, nunca de `@katajs/core`
...
| Providers assíncronos & lifecycle hooks (`OnModuleInit`, `OnApplicationShutdown`) | Não há container para conduzir um lifecycle | Factories `singleton` eager na inicialização; teardown via `gracefulShutdown` (`@katajs/core/node`) em `main.ts` ([ADR-0014](/adr/0014-lifecycle-shutdown), [database.md](/pt/cookbook/database#closing-the-pool-on-shutdown)) |
