...
# ou: pnpm add @katajs-framework/core hono zod @hono/node-server
```
...
Kata vem com auth JWT sob `@katajs-framework/core/jwt`. `jwtAuth` lê um token `Authorization: Bearer`,
