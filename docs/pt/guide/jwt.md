...
`@katajs-framework/core/jwt` entrega os blocos de construção para esse fluxo, para que você não tenha que escrever um verificador na mão:
**assinar** um token, **verificá-lo**, **autenticar** uma requisição em um scoped slot e
**autorizar** esse slot com guards. Não adiciona **nenhuma dependência nova** — `@katajs-framework/core/jwt` é um
...
`@katajs-framework/core/jwt` é a costura de verificar-e-autorizar; o modelo de credencial e
...
seu predicado diz não. `requireRole`, `requireClaim` e `guard` retornam todos um
handler puro — não um `Middleware<R>` — então, exatamente como o [`guard`](#guard)
abaixo, quem chama envolve com `defineMiddleware({ provides: [] as const, handler })`.
A ordem no array `use:` é o contrato: o guard precisa vir **depois** do middleware
que preenche o slot.
...
`@katajs-framework/core/jwt` para deliberadamente na fronteira de verificar-e-autorizar. O modelo
...
- [Referência de `@katajs-framework/core/jwt`](/pt/reference/jwt) — assinaturas completas.
