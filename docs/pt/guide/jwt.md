...
`@katajs/core/jwt` entrega os blocos de construção para esse fluxo, para que você não tenha que escrever um verificador na mão:
**assinar** um token, **verificá-lo**, **autenticar** uma requisição em um scoped slot e
**autorizar** esse slot com guards. Não adiciona **nenhuma dependência nova** — `@katajs/core/jwt` é um
...
`@katajs/core/jwt` é a costura de verificar-e-autorizar; o modelo de credencial e
...
`@katajs/core/jwt` para deliberadamente na fronteira de verificar-e-autorizar. O modelo
...
- [Referência de `@katajs/core/jwt`](/pt/reference/jwt) — assinaturas completas.
