...
`gracefulShutdown` vive no subpath **`@katajs-framework/core/node`** — o único ponto de entrada que
toca em `node:process` — para que importar o core neutro em relação ao runtime
(`@katajs-framework/core`) de um build edge ou Workers nunca puxe o Node junto.
...
para que `@katajs-framework/core/node` precise apenas de `@types/node` e nunca empacote o adaptador.
...
gracefulShutdown(server, {
  onClose: async () => {
    ...
    await k.resolve('db').close() // por último, fecha o pool
  },
})
```
...
`@katajs-framework/core/node` é exclusivo de Node e é o único adaptador de ciclo de vida da v0.3.
