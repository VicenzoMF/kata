...
`gracefulShutdown` vive no subpath **`@katajs/core/node`** — o único ponto de entrada que
toca em `node:process` — para que importar o core neutro em relação ao runtime
(`@katajs/core`) de um build edge ou Workers nunca puxe o Node junto.
...
para que `@katajs/core/node` precise apenas de `@types/node` e nunca empacote o adaptador.
...
`@katajs/core/node` é exclusivo de Node e é o único adaptador de ciclo de vida da v0.3.
