...
framework: sem `@katajs/core`, sem Hono, sem `defineContext`, sem `defineRoute`, sem `c`. Um handler de
...
**não** importa: nada de `@katajs/core`, nenhum Hono, nenhum contexto de requisição.
...
- Um service importa tipos de `<domain>.schema.ts` e de outros services. Nada
  de `@katajs/core`, Hono ou do contexto de requisição.
