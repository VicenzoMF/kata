...
Um service guarda a sua lógica de negócio. Ele vive em `<domain>.service.ts` e
contém funções comuns — sem imports de framework, sem `defineContext`, sem
`defineRoute`, sem `c`. Um handler de route valida o input, chama um service e
retorna o resultado. O service não sabe que está sendo servido sobre HTTP.

Essa separação é o sentido do layout travado: a route é o contrato, o
service é a lógica, o schema é o formato. Cada um é encontrável por glob e
testável por conta própria.
...
recebem e retornam esses DTOs tipados. Essa é toda a superfície de dependência.
...
A route detém as preocupações de HTTP — método, path, `input`, `output`, status
codes — e delega o trabalho. O handler lê o `c.input` tipado, chama o
service e retorna o valor (que o Kata valida contra `output`):
...
O handler é um adaptador fino: desempacota o `c.input`, chama o service, mapeia o
resultado em uma resposta. Mantenha a validação nos schemas e a lógica no service,
e o handler permanece pequeno. Veja [/pt/guide/routes-schemas](/pt/guide/routes-schemas)
para a superfície completa de route.

## Dependências são argumentos, não imports

O store do `hello` é um `Map` em nível de módulo — ótimo para uma demo, mas um service
real precisa de um banco de dados, e um service precisa permanecer livre do container de DI para se manter puro.
A regra: **um service nunca chama `c.get(...)`.** Ele recebe suas dependências como
argumentos. A route as puxa do contexto e as passa adiante.
...
A route conecta a dependência. `store` é um slot singleton em
...
`c.get('store')` e `c.get('currentUser')` vivem no handler, onde o
contexto existe. O service enxerga apenas um `Store` e um `userId`. Troque o store
por um banco de dados real e a assinatura do service não muda — apenas o
singleton que você registra.

## Retorne resultados, não respostas

Um service não tem `c`, então não pode chamar `c.json(...)` nem `c.error(...)`. Quando uma
operação pode falhar de um jeito sobre o qual o chamador precisa ramificar, retorne uma
**união discriminada** tipada e deixe a route traduzi-la em um status code.
...
O mapeamento em si — `describeCheckoutFailure` — é outra função pura no
mesmo service, então o contrato de erro também é testável em unidade. Os
helpers específicos de HTTP (`c.error`, o `201`) permanecem na route. Veja
[/pt/guide/errors](/pt/guide/errors) para o envelope.

## Services são trivialmente testáveis

Como um service não importa framework nenhum e recebe suas dependências como argumentos,
o teste dele importa as funções e as chama. Nenhum app para iniciar, nenhuma requisição para
forjar, nenhum mock de `c`. O arquivo de teste é `<domain>.service.test.ts`, o vizinho
do service.
...
um catálogo semente, depois faz asserções sobre a união retornada e o estado do store:
...
de erro — é exercitada sem HTTP. Rode-a com `pnpm test`.

::: info Puro por construção
Manter `c.get(...)` na route e fora do service é o que torna isso
possível. Se um service alcançasse o container, você teria que forjar o
container para testá-lo. Passar a dependência adiante mantém o teste como uma simples chamada de
função.
:::

## Persistência é bring-your-own

O Kata não traz uma camada de banco de dados, um ORM ou helpers de query — persistência é um
não-objetivo ([/pt/cookbook/non-goals](/pt/cookbook/non-goals)). Ambos os exemplos usam um
store em memória como substituto. O padrão que mantém a troca indolor:
modele o seu acesso a dados como um cliente tipado, registre-o como um slot `singleton` e passe-o
para os services como argumento.

Quando você substituir o `Store` em memória por node-postgres, Drizzle, Prisma ou
qualquer outra coisa, apenas o singleton que você registra em `defineContext` muda — as
assinaturas de service e os handlers de route permanecem os mesmos. A receita, de ponta a ponta,
está em [/pt/cookbook/database](/pt/cookbook/database).
...
Veja também: [/pt/guide/routes-schemas](/pt/guide/routes-schemas),
[/pt/guide/context-di](/pt/guide/context-di), [/pt/guide/project-layout](/pt/guide/project-layout).
