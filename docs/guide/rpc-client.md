...
::: tip
`KataApp` is exported from `@katajs-framework/core` if you want to name the type explicitly:
...
`ErrorBodySchema` (exported from `@katajs-framework/core`) is the canonical schema for Kata's error
