...
::: tip
`KataApp` is exported from `@katajs/core` if you want to name the type explicitly:
...
`ErrorBodySchema` (exported from `@katajs/core`) is the canonical schema for Kata's error
