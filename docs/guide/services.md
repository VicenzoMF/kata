...
framework: no `@katajs/core`, no Hono, no `defineContext`, no `defineRoute`, no `c`. A route
...
**not** import: nothing from `@katajs/core`, no Hono, no request context.
...
- A service imports types from `<domain>.schema.ts` and other services. Nothing from
  `@katajs/core`, Hono, or the request context.
