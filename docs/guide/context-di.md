...
mechanism. The generic `defineRoute` you could import from `@katajs/core` knows nothing
...
| Import from `@katajs/core` (generic)           | Import from your `context.ts` (bound)          |
|----------------------------------------|------------------------------------------------|
...
**The mistake to avoid:** importing `defineRoute` straight from `@katajs/core`, or calling
