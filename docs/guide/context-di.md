...
mechanism. The generic `defineRoute` you could import from `@katajs-framework/core` knows nothing
...
| Import from `@katajs-framework/core` (generic)           | Import from your `context.ts` (bound)          |
|----------------------------------------|------------------------------------------------|
...
**The mistake to avoid:** importing `defineRoute` straight from `@katajs-framework/core`, or calling
