...
orchestrator asks. That is `gracefulShutdown` from `@katajs/core/node`.
...
`gracefulShutdown` lives in the **`@katajs/core/node`** subpath — the only entry that
touches `node:process` — so importing the runtime-neutral core (`@katajs/core`) from an
edge or Workers build never pulls Node in.

```ts
import { gracefulShutdown } from '@katajs/core/node'
import type { ServerType, GracefulShutdownOptions } from '@katajs/core/node'
...
`@types/node` so `@katajs/core/node` needs only `@types/node` and never bundles the
adapter.
...
`@katajs/core/node` is Node-only and is v0.3's only lifecycle adapter. Edge and Workers
