...
orchestrator asks. That is `gracefulShutdown` from `@katajs-framework/core/node`.
...
`gracefulShutdown` lives in the **`@katajs-framework/core/node`** subpath — the only entry that
touches `node:process` — so importing the runtime-neutral core (`@katajs-framework/core`) from an
edge or Workers build never pulls Node in.

```ts
import { gracefulShutdown } from '@katajs-framework/core/node'
import type { ServerType, GracefulShutdownOptions } from '@katajs-framework/core/node'
...
`@types/node` so `@katajs-framework/core/node` needs only `@types/node` and never bundles the
adapter.
...
`@katajs-framework/core/node` is Node-only and is v0.3's only lifecycle adapter. Edge and Workers
