...
them so the rest of the app imports from `./context`, never from `@katajs/core`
...
| Async providers & lifecycle hooks (`OnModuleInit`, `OnApplicationShutdown`) | No container to drive a lifecycle | Eager `singleton` factories at startup; teardown via `gracefulShutdown` (`@katajs/core/node`) in `main.ts` ([ADR-0014](../adr/0014-lifecycle-shutdown.md), [database.md](./database.md#closing-the-pool-on-shutdown)) |
