...
> **Before you start.** Install Kata with
> `npm install @katajs-framework/core hono zod @hono/node-server`, or clone the
> repo and copy [`examples/hello`](https://github.com/VicenzoMF/kata/tree/main/examples/hello) into
...
them so the rest of the app imports from `./context`, never from `@katajs-framework/core`
...
| Async providers & lifecycle hooks (`OnModuleInit`, `OnApplicationShutdown`) | No container to drive a lifecycle | Eager `singleton` factories at startup; teardown via `gracefulShutdown` (`@katajs-framework/core/node`) in `main.ts` ([ADR-0014](../adr/0014-lifecycle-shutdown.md), [database.md](./database.md#closing-the-pool-on-shutdown)) |
